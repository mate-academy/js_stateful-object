...
test('Should add multiple properties to non empty state', () => {
  ...
  expect(state)
    .toEqual({ foo: 'bar', bar: 'foo', name: 'Jim', hello: 'world' });
});

test('Should replace existing properties', () => {
  const state = { foo: 'bar', bar: 'foo' };
  transformState(state, [
    { operation: 'addProperties', properties: { foo: 'new', hello: 'world' } },
  ]);
  expect(state)
    .toEqual({ foo: 'new', bar: 'foo', hello: 'world' });
});
...
test('Should remove several properties', () => {
  const state = { foo: 'bar', bar: 'foo', name: 'Jim', hello: 'world' };
  transformState(state, [
...
test('Should not fail when removing not existing property', () => {
  ...
  expect(state)
    .toEqual({ foo: 'bar', name: 'Jim', hello: 'world' });
});
